...
| 4 | Fielding, catching, run outs, DRS in full detail; re-calibrate | **Done — DRS, run outs at both ends, direct hits, catch grading, the bat-edge fix (§16), the running rewrite (§17) and the missing wide line (§18). T20 meets all thirteen bands; the four-day run rate is in band for the first time. The two gaps that remain are recorded with their measured causes in `docs/CALIBRATION.md`: the List A survival hazard does not fall as a batter settles, and the longer formats' boundary bands cannot be met alongside their dot and run-rate bands** |
...
| 7 | Save/load, multiple careers, difficulty, editable database, records, retirement | **In progress — editable seed database, fixtures, ladder, progression and retirement done; save/load, multiple careers, difficulty and records outstanding** |
