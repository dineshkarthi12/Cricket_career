...
| 4 | Fielding, catching, run outs, DRS in full detail; re-calibrate | **Done — DRS, run outs at both ends, direct hits, catch grading, the bat-edge fix (§16), the running rewrite (§17) and the missing wide line (§18). T20 meets all thirteen bands; the four-day run rate is in band for the first time. The two gaps that remain are recorded with their measured causes in `docs/CALIBRATION.md`: the longer formats' boundary bands cannot be met alongside their dot and run-rate bands, and the six-to-four split (`docs/SIMULATION_MODEL.md` §19). The List A survival hazard is fixed — it was the field, not the settling model** |
